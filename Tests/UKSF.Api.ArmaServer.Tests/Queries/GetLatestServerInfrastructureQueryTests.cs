...
namespace UKSF.Api.ArmaServer.Tests.Queries
{
    public class GetLatestServerInfrastructureQueryTests
    {
        ...
        [Fact]
        public void When_getting_latest_server_info_from_steam_with_no_info()
        {
            Given_steam_cmd_returns_no_info();

            Func<Task> act = async () => await _subject.ExecuteAsync();

            act.Should().ThrowAsync<ServerInfrastructureException>().WithMessageAndStatusCode("No info found from Steam", 404);
        }

        [Fact]
        public void When_getting_latest_server_info_from_steam_with_bad_info()
        {
            Given_steam_cmd_returns_bad_info();

            Func<Task> act = async () => await _subject.ExecuteAsync();

            act.Should().ThrowAsync<ServerInfrastructureException>().WithMessageAndStatusCode("No build info found in Steam data", 404);
        }
