...
) : IWorkshopModsProcessingService
{
    ...
    public async Task QueueDevBuild(string workshopModName, WorkshopModStatus workshopModStatus)
    {
        ...
        try
        {
            ...
            await modpackService.NewBuild(new NewBuild { Reference = "main", Changes = BuildWorkshopModChanges(workshopModName, workshopModStatus) });
        }
        catch (Exception exception)
        {
    ...
    /// <summary>
    ///     The build takes whatever main already holds, so its commit message describes code that earlier builds already
    ///     shipped. The workshop mod is the only new content, so it is named as the build's changes instead.
    /// </summary>
    private static string BuildWorkshopModChanges(string workshopModName, WorkshopModStatus workshopModStatus)
    {
        var section = workshopModStatus switch
        {
            WorkshopModStatus.InstalledPendingRelease => "Added",
            WorkshopModStatus.UpdatedPendingRelease   => "Updated",
            _                                         => "Removed"
        };
