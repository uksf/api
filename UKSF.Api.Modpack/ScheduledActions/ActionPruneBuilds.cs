...
public class ActionPruneBuilds : SelfCreatingScheduledAction, IActionPruneBuilds
{
    ...
    {
        var devBuilds = _buildsContext.Get(x => x.Environment == GameEnvironment.Development).Select(x => x.BuildNumber).ToList();
        if (devBuilds.Count == 0)
        {
            return;
        }

        var threshold = devBuilds.Max() - 100;
