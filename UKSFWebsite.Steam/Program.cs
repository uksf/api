...
namespace UKSFWebsite.Steam {
    internal class Program {
        ...
        private static IWebHost BuildWebHost(string[] args) =>
            WebHost.CreateDefaultBuilder(args).UseStartup<Startup>().UseKestrel().UseContentRoot(Directory.GetCurrentDirectory()).UseUrls("http://*:5100").UseIISIntegration().Build();
#else
        private static IWebHost BuildWebHost(string[] args) =>
        ...
        private static void InitLogging() {
            string appdata = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "UKSFWebsiteSteam");
