...
//             ModpackReleaseCandidate subject = new ModpackReleaseCandidate { version = VERSION, builds = new List<ModpackBuild> { new ModpackBuild { buildNumber = 0, isNewVersion = true } } };
//             List<ModpackReleaseCandidate> data = new List<ModpackReleaseCandidate> { subject };
//             GithubCommit commit = new GithubCommit();
//
//             mockBuildsDataService.Setup(x => x.GetSingle(It.IsAny<Func<ModpackReleaseCandidate, bool>>())).Returns<Func<ModpackReleaseCandidate, bool>>(x => data.FirstOrDefault(x));
...
//             GithubCommit commit = new GithubCommit();
//             ModpackReleaseCandidate subject = null;
//
//             mockBuildsDataService.Setup(x => x.GetSingle(It.IsAny<Func<ModpackReleaseCandidate, bool>>())).Returns<Func<ModpackReleaseCandidate, bool>>(null);
//             mockBuildsDataService.Setup(x => x.Add(It.IsAny<ModpackReleaseCandidate>())).Returns(Task.CompletedTask).Callback<ModpackReleaseCandidate>(x => subject = x);
//
//             ModpackBuild buildSubject = await buildsService.CreateDevBuild(VERSION, commit);
//
...
//             ModpackBuild build = new ModpackBuild { buildNumber = 4 };
//             ModpackReleaseCandidate subject = new ModpackReleaseCandidate { version = VERSION, builds = new List<ModpackBuild> { build } };
//
//             mockBuildsDataService.Setup(x => x.GetSingle(It.IsAny<Func<ModpackReleaseCandidate, bool>>())).Returns(subject);
//             mockBuildsDataService.Setup(x => x.Update(It.IsAny<string>(), It.IsAny<ModpackBuild>(), It.IsAny<DataEventType>()))
//                                  .Returns(Task.CompletedTask)
//                                  .Callback<string, ModpackBuild, DataEventType>((x1, x2, x3) => subject.builds.Add(x2));
//
