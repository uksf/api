...
namespace UKSFWebsite.Api.Services {
    public class AttendanceService : IAttendanceService {
        ...
        public async Task<AttendanceReport> GenerateAttendanceReport(DateTime start, DateTime end) {
            ...
            AccountAttendanceStatus[] reports = accounts.Select(
                                                            x => new AccountAttendanceStatus {
                                                                ...
                                                                attendanceState =
                                                                    loaService.IsLoaCovered(x.id, start) ? AttendanceState.LOA : GetAttendanceState(GetAttendancePercent(x.teamspeakIdentities)),
                                                                groupId = unitsService.GetSingle(y => y.name == x.unitAssignment).id,
        ...
        private void GetAccounts() {
            accounts = accountService.Get(x => x.membershipState == MembershipState.MEMBER);
        }
        ...
        private static AttendanceState GetAttendanceState(float attendancePercent) =>
            attendancePercent > 0.6 ? AttendanceState.FULL : attendancePercent > 0.3 ? AttendanceState.PARTIAL : AttendanceState.MIA;
    }
}
