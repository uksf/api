...
namespace UKSFWebsite.Api.Services.Utility {
    public static class GameServerHelpers {
        ...
        public static string FormatHeadlessClientLaunchArguments(this GameServer gameServer, int index) =>
            $"-profiles={GetGameServerProfilesPath()} -name={GetHeadlessClientName(index)} -port={gameServer.port} -apiport=\"{gameServer.apiPort + index + 1}\" -mod={gameServer.FormatGameServerMods()} -localhost=127.0.0.1 -connect=localhost -password={gameServer.password} -client -nosound -enableHT -high -hugepages -filePatching -pid=\"{Path.Join(GetGameServerProfilesPath(), $"{gameServer.name}_{GetHeadlessClientName(index)}.pid")}\"";
