...
namespace UKSFWebsite.Api.Services.Missions {
    public class MissionPatchingService : IMissionPatchingService {
        ...
        private void CreateBackup() {
            string backupPath = Path.Combine(VariablesWrapper.VariablesService().GetSingle("MISSION_BACKUPS_FOLDER").AsString(), Path.GetFileName(filePath) ?? throw new FileNotFoundException());
        ...
        private async Task PackPbo() {
            ...
            Process process = new Process {
                StartInfo = {FileName = MAKE_PBO, WorkingDirectory = VariablesWrapper.VariablesService().GetSingle("MAKEPBO_WORKING_DIR").AsString(), Arguments = $"-Z -BD -P -X=thumbs.db,*.txt,*.h,*.dep,*.cpp,*.bak,*.png,*.log,*.pew \"{folderPath}\"", UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true}
            };
            process.Start();
