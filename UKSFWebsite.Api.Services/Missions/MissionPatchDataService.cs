...
namespace UKSFWebsite.Api.Services.Missions {
    public class MissionPatchDataService {
        ...
        public void UpdatePatchData() {
            MissionPatchData.instance = new MissionPatchData {units = new List<MissionUnit>(), ranks = ranksService.Get(), players = new List<MissionPlayer>(), orderedUnits = new List<MissionUnit>()};

            foreach (Unit unit in unitsService.Get(x => x.branch == UnitBranch.COMBAT).ToList()) {
                MissionPatchData.instance.units.Add(new MissionUnit {sourceUnit = unit, depth = unitsService.GetUnitDepth(unit)});
            }

            foreach (Account account in accountService.Get().Where(x => !string.IsNullOrEmpty(x.rank) && ranksService.IsSuperiorOrEqual(x.rank, "Recruit"))) {
                MissionPatchData.instance.players.Add(new MissionPlayer {account = account, rank = ranksService.GetSingle(account.rank), name = displayNameService.GetDisplayName(account)});
            }
