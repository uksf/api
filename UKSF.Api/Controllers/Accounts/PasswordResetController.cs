...
namespace UKSF.Api.Controllers.Accounts {
    ...
    public class PasswordResetController : ConfirmationCodeReceiver {
        ...
        public PasswordResetController(IConfirmationCodeService confirmationCodeService, ILoginService loginService, IEmailService emailService, IAccountService accountService) : base(
        ...
        public async Task<IActionResult> ResetPassword([FromBody] JObject body) {
            ...
            LogWrapper.AuditLog($"Password reset request made for {account.id}", account.id);
