...
namespace UKSF.Api.Personnel.Controllers
{
    ...
    public class CommunicationsController : Controller
    {
        ...
        [HttpGet, Authorize]
        public IActionResult GetTeamspeakStatus()
        {
            return Ok(new { isConnected = _accountService.GetUserAccount().TeamspeakIdentities?.Count > 0 });
        }
        ...
        private async Task<IActionResult> ReceiveTeamspeakCode(string id, string code, string checkId)
        {
            ...
            account.TeamspeakIdentities ??= new();
